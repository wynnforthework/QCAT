...
func (me *MockExecutor) Execute(ctx context.Context, params map[string]interface{}) (interface{}, error) {
	...
	// 模拟失败
	if me.simulateFailure && rand.Float32() < 0.1 { // 10% 失败率
...
func CreateDefaultExecutors() map[int]AutomationExecutor {
	...
	for id, name := range functionNames {
		...
		switch {
		case id >= 24: // 学习进化类功能
			execTime = time.Duration(10+rand.Intn(20)) * time.Second
		case id >= 18: // 数据分析类功能
			execTime = time.Duration(5+rand.Intn(10)) * time.Second
		case id >= 12: // 风险安全类功能
			execTime = time.Duration(1+rand.Intn(5)) * time.Second
		default: // 交易策略类功能
			execTime = time.Duration(2+rand.Intn(8)) * time.Second
		}
