...
func (s *Strategy) OnKline(ctx context.Context, k *kline.Kline) error { s.closes = append(s.closes, k.Close); if len(s.closes) < s.period+1 { return nil }; rsi := calcRSI(s.closes, s.period); if s.exec != nil { var side string; if rsi < s.buy { side = "BUY" } else if rsi > s.sell { side = "SELL" } if side != "" { _ = s.exec.PlaceOrder(&exchange.Order{ ID: k.Symbol+"-"+k.OpenTime.Format("20060102150405"), Symbol: k.Symbol, Side: side, Type: string(exchange.OrderTypeMarket), Price: k.Close, Quantity: 1.0, CreatedAt: k.OpenTime }) } }; return nil }
