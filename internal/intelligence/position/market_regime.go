...
// DetectRegime 检测市场状态
func (mrd *MarketRegimeDetector) DetectRegime(data *types.Kline) MarketRegime {
	// 基于单个K线数据进行简化的市场状态检测
	if data.Close == 0 || data.Open == 0 {
		return RegimeUncertain
	}

	// 1. 计算价格变化率作为趋势强度的简化指标
	priceChange := (data.Close - data.Open) / data.Open
	trendStrength := math.Abs(priceChange)

	// 2. 计算波动率水平（基于单个K线的简化计算）
	...
	// 4. 计算市场压力指标（基于成交量）
	stress := data.Volume / 1000000 // 简化的压力指标
...
func (vp *VolatilityPredictor) PredictVolatility(data *types.Kline) float64 {
	// 基于单个K线数据的简化波动率计算
	if data.High == 0 || data.Low == 0 || data.Open == 0 {
		return 0.0
	}

	// 使用简化的波动率计算：(High - Low) / Open
	volatility := (data.High - data.Low) / data.Open
