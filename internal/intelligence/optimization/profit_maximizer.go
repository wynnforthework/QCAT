...
func (ra *RiskAdjuster) AdjustForRisk(
	...
	for symbol, weight := range allocation {
		if math.IsNaN(weight) || math.IsInf(weight, 0) {
			log.Printf("Warning: Invalid weight for %s, using 0.0", symbol)
			weight = 0.0
		}
	...
	// 如果总权重为0或无效，使用均等分配
	if totalWeight <= 0 || math.IsNaN(totalWeight) {
		log.Printf("Warning: Invalid total weight, using equal allocation")
		if len(adjustedAllocation) > 0 {
			equalWeight := 1.0 / float64(len(adjustedAllocation))
			for symbol := range adjustedAllocation {
				adjustedAllocation[symbol] = equalWeight
			}
		}
	}
	...
	// 基于当前状态调整风险指标
	if currentState != nil && len(currentState.Positions) > 0 {
		// 简化的风险计算，避免NaN
		returnSum := 0.0
		count := 0

		for _, pos := range currentState.Positions {
			if !math.IsNaN(pos.UnrealizedPnL) && !math.IsInf(pos.UnrealizedPnL, 0) {
				returnSum += pos.UnrealizedPnL
				count++
			}
		}

		if count > 0 {
			avgReturn := returnSum / float64(count)
			if !math.IsNaN(avgReturn) && !math.IsInf(avgReturn, 0) {
				expectedReturn = math.Max(0.01, math.Min(0.5, avgReturn))
			}
		}
	}
...
func (pme *ProfitMaximizationEngine) calculateObjectiveValue(
	...
	if math.IsNaN(expectedReturn) || math.IsInf(expectedReturn, 0) {
		log.Printf("Warning: Invalid expected return, using default value 0.0")
		expectedReturn = 0.0
	}

	if math.IsNaN(portfolioVolatility) || math.IsInf(portfolioVolatility, 0) || portfolioVolatility < 0 {
		log.Printf("Warning: Invalid portfolio volatility, using default value 0.1")
		portfolioVolatility = 0.1
	}

	if math.IsNaN(totalCosts) || math.IsInf(totalCosts, 0) || totalCosts < 0 {
		log.Printf("Warning: Invalid total costs, using default value 0.0")
		totalCosts = 0.0
	}

	// 效用函数: Return - λ*Risk - γ*Cost
	utility := expectedReturn -
		pme.config.RiskWeight*portfolioVolatility -
		pme.config.CostWeight*totalCosts/10000 // 标准化成本

	// 最终验证结果
	if math.IsNaN(utility) || math.IsInf(utility, 0) {
		log.Printf("Warning: Calculated utility is NaN/Inf, returning default value 0.0")
		return 0.0
	}
