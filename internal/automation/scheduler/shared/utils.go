...
// CalculateSharpeRatio calculates the Sharpe ratio with improved edge case handling
func CalculateSharpeRatio(returns []float64, riskFreeRate float64) float64 {
	...
	// 处理标准差为0的情况
	if stdDev == 0 {
		// 如果所有收益都相同且为正，返回一个合理的正值
		if meanReturn > riskFreeRate {
			return 1.0 // 返回一个保守的正值
		}
		return 0
	}

	// 处理极小的标准差（可能导致极大的Sharpe比率）
	if stdDev < 1e-8 {
		if meanReturn > riskFreeRate {
			return 2.0 // 限制最大Sharpe比率
		}
		return 0
	}
